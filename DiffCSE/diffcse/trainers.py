...
# import wandb
# wandb.login()
# wandb.init(project="KoDiffCSE", name='KoDiffCSE')
...
class CLTrainer(Trainer):
    ...
    def evaluate(
        ...
        valid_setup = contentDataset(file = file_path, tok = self.tokenizer, max_len = 512)
        valid_dataloader = DataLoader(valid_setup, batch_size=32, shuffle=False)

        self.model.eval()
        
        sent1_pooler_output_list = None
        sent2_pooler_output_list = None
        labels_list = None

        i = 0
        for batch in valid_dataloader:
            batch = {k: v.to(self.args.device) for k, v in batch.items()}
            sent_1 = batch['text1_encoder_input_ids']
            sent_2 = batch['text2_encoder_input_ids']

            sent_1_attention_mask = batch["text1_encoder_input_ids"].ne(0).float().to(self.args.device)
            sent_2_attention_mask = batch["text2_encoder_input_ids"].ne(0).float().to(self.args.device)

            with torch.no_grad():
                sent1_output = self.model(sent_1, attention_mask = sent_1_attention_mask, output_hidden_states=True, return_dict=True, sent_emb=True)
                sent2_output = self.model(sent_2, attention_mask = sent_2_attention_mask, output_hidden_states=True, return_dict=True, sent_emb=True)
                # sent1_output = self.model(sent_1, attention_mask = sent_1_attention_mask, output_hidden_states=True, return_dict=True, sent_emb=False)
                # sent2_output = self.model(sent_2, attention_mask = sent_2_attention_mask, output_hidden_states=True, return_dict=True, sent_emb=False)

                sent1_pooler_output = sent1_output.pooler_output
                sent2_pooler_output = sent2_output.pooler_output
                labels = batch['score']

                if i == 0:
                    sent1_pooler_output_list = sent1_pooler_output.cpu()
                    sent2_pooler_output_list = sent2_pooler_output.cpu()
                    labels_list = labels.cpu()
                else:
                    sent1_pooler_output_list = torch.cat((sent1_pooler_output_list, sent1_pooler_output.cpu()),dim=0)
                    sent2_pooler_output_list = torch.cat((sent2_pooler_output_list, sent2_pooler_output.cpu()),dim=0)
                    labels_list = torch.cat((labels_list, labels.cpu()),dim=0)
                    
                # sent1_pooler_output_list.append(sent1_pooler_output)
                # sent2_pooler_output_list.append(sent2_pooler_output)
                # labels_list.append(labels)

            i +=1

        def evaluation(sent1_pooler_output, sent2_pooler_output, labels):

            sen_emb1 = sent1_pooler_output.cpu()
            sen_emb2 = sent2_pooler_output.cpu()
        ...
        score = evaluation(sent1_pooler_output_list, sent2_pooler_output_list, labels_list)
    ...
    def train(self, model_path: Optional[str] = None, trial: Union["optuna.Trial", Dict[str, Any]] = None):
        ...
        for epoch in range(epochs_trained, num_train_epochs):
            ...
            for step, inputs in enumerate(epoch_iterator):
                ...
                tr_pos_sim += model.module.sim.pos_avg
                tr_neg_sim += model.module.sim.neg_avg
                tr_others["electra_acc"] += model.module.electra_acc
                tr_others["electra_rep_acc"] += model.module.electra_rep_acc
                tr_others["electra_fix_acc"] += model.module.electra_fix_acc
        ...
        # wandb.log({'_total_loss_scalar' : self._total_loss_scalar,
        #           '_total_pos_sim_scalar' : self._total_pos_sim_scalar,
        #             '_total_neg_sim_scalar' : self._total_neg_sim_scalar,})
